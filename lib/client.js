    ...
    class WorkspacePersonaSection extends React.Component {
      ...
      workspaceItems() {
        ...
        return workspaces.map((workspace) => ({
          id: workspace.path,
          label: workspace.sessionCount > 0 ? `${workspace.title} · ${workspace.sessionCount} 个会话` : workspace.title,
      ...
      sessionItems() {
        ...
        for (const session of sessions.slice(0, 10)) items.push({ id: session.id, label: label(session), icon: h(IconListPenOutline16, {}) })
            ...
            return h('div', { className: 'wsp-scope-row', key: `t${index}` }, [
              ...
              isCustom
                ...
                : h(Menu, {
                  ...
                  ...(target.value === '' ? {} : { selectedId: target.value }),
                  ...
                  anchor: h('button', {
                    type: 'button',
                    className: picked === undefined ? 'wsp-picker wsp-picker-empty' : 'wsp-picker',
                    'aria-haspopup': 'menu',
                    'aria-expanded': state.picker === key,
                    onClick: () => this.togglePicker(key),
                  }, [
                    h(target.kind === 'workspace' ? IconFolderOpen16 : IconListPenOutline16, { key: 'i' }),
                    h('span', { className: 'wsp-picker-label', key: 'l', title: picked?.label }, picked?.label ?? (target.kind === 'workspace' ? '选择一个工作区…' : '选择一条会话…')),
