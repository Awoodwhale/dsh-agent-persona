...
.wsp-foot-action { display: inline-flex; align-items: center; gap: 6px; height: 28px; padding: 0 10px; border: 1px solid var(--wsp-line); border-radius: 8px; background: var(--wsp-surface-2); font: inherit; font-size: 12.5px; color: var(--wsp-muted); cursor: pointer; transition: color 150ms ease, background 150ms ease; }
.wsp-foot-action:hover { color: var(--wsp-text); background: var(--wsp-hover); }
.wsp-overlay { position: fixed; inset: 0; z-index: 40; display: flex; align-items: center; justify-content: center; padding: 4vh 3vw; background: rgba(0, 0, 0, 0.35); }
.wsp-overlay-card { display: flex; flex-direction: column; width: min(1100px, 94vw); height: min(720px, 88vh); border: 1px solid var(--wsp-line); border-radius: 16px; background: var(--wsp-surface); box-shadow: 0 18px 48px rgba(0, 0, 0, 0.28); overflow: hidden; }
.wsp-overlay-head { display: flex; align-items: center; gap: 10px; padding: 12px 16px; border-bottom: 1px solid var(--wsp-line-soft); }
.wsp-overlay-title { font-size: 13px; font-weight: 600; color: var(--wsp-text); }
.wsp-overlay-head > *:last-child { margin-left: auto; }
.wsp-overlay-body { flex: 1 1 auto; min-height: 0; display: flex; }
...
.wsp-panel-area { display: flex; flex-direction: column; gap: 16px; }
...
`
    ...
    /** Open state shared by the sidebar footer button and the floating panel it opens. */
    let overlayOpen = false
    const overlayListeners = new Set()
    const setOverlayOpen = (open) => {
      overlayOpen = open
      for (const listener of overlayListeners) listener(open)
    }

    /** The sidebar-foot button that opens the persona panel. */
    class PersonaFooterAction extends React.Component {
      constructor(props) {
        super(props === undefined || props === null ? {} : props)
        this.state = { open: overlayOpen }
      }

      componentDidMount() {
        this.listener = (open) => this.setState({ open })
        overlayListeners.add(this.listener)
      }

      componentWillUnmount() {
        overlayListeners.delete(this.listener)
      }

      render() {
        const label = String(this.props?.label ?? '人设')
        return h('button', {
          type: 'button',
          className: 'wsp-foot-action',
          title: this.state.open ? '收起人设面板' : '打开人设面板：看这条会话的人设与完整提示词，并管理全部人设',
          'aria-pressed': this.state.open,
          onClick: () => setOverlayOpen(!overlayOpen),
        }, [
          h(IconPersonalizationOutline16, { key: 'i' }),
          h('span', { key: 'l' }, label),
        ])
      }
    }

    /** The floating panel the footer button opens; the persona view fills it. */
    class PersonaOverlay extends React.Component {
      constructor(props) {
        super(props === undefined || props === null ? {} : props)
        this.state = { open: overlayOpen }
      }
      ...
      render() {
        ...
        return h('div', { className: 'wsp-overlay', 'data-plugin': NS }, [
          h('div', { className: 'wsp-overlay-card', key: 'card' }, [
            h('div', { className: 'wsp-overlay-head', key: 'head' }, [
              h('span', { className: 'wsp-overlay-title', key: 't' }, '人设'),
              h(Button, { key: 'close', variant: 'outline', size: 'sm', onClick: () => setOverlayOpen(false) }, '关闭'),
            ]),
            h('div', { className: 'wsp-overlay-body', key: 'body' },
              h(PersonaView, { api: this.props?.api ?? capturedApi })),
          ]),
        ])
      }
    }
    ...
    class PersonaView extends React.Component {
      ...
      render() {
        ...
        if (loading) {
          ...
          panel = h('div', { className: 'wsp-panel-area wyst', key: 'manage' }, [
            h('p', { className: 'wsp-card-note', key: 'note' },
              '这里管理全部人设：新建、编辑正文、调整适用范围与优先级、复制或删除。改动保存在同一份存储里，对之后的所有请求生效；「人设」标签只看这条会话当前命中的那一条。'),
...
const since = (timestamp) => {
  ...
  return `${Math.round(seconds / 86400)} 天前`
    ...
    function apply(ctx) {
      ...
      // The sidebar entry, structured like dsh-context's: a footer action that opens a
      // floating panel. The two share a tiny store, so the button can toggle the panel.
