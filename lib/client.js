...
.wsp-tabs { display: flex; align-items: flex-end; gap: 2px; max-width: 1040px; width: 100%; margin: 0 auto; border-bottom: 1px solid var(--wsp-line); }
.wsp-tab { position: relative; padding: 8px 14px 10px; border: 0; border-radius: 8px 8px 0 0; background: none; font: inherit; font-size: 13px; color: var(--wsp-muted); cursor: pointer; transition: color 150ms ease, background 150ms ease; }
.wsp-tab:hover { color: var(--wsp-text); background: var(--wsp-hover); }
.wsp-tab:focus-visible { outline: 2px solid var(--wsp-accent); outline-offset: 1px; }
.wsp-tab-on { color: var(--wsp-accent); font-weight: 600; }
.wsp-tab-on::after { content: ''; position: absolute; left: 10px; right: 10px; bottom: -1px; height: 2px; border-radius: 2px; background: var(--wsp-accent); }
...
.wsp-card-panel:hover { box-shadow: var(--dsw-shadow-lv3, 0 6px 20px rgba(0, 0, 0, 0.14)); }
      ...
      render() {
        ...
        const header = h('header', { className: 'wsp-view-head', key: 'head' }, [
          ...
          h('div', { className: 'wsp-view-headactions', key: 'a' }, [
            h(Button, { key: 'reload', variant: 'outline', size: 'sm', disabled: busy === 'load', onClick: () => void this.load() },
              busy === 'load' ? '读取中…' : '重新读取'),
          ]),
        ])
        ...
        if (loading) {
          ...
          panel = h('div', { className: 'wsp-panel-area', key: 'persona' }, card({
            ...
            extra: [
              ...
              h('p', { className: 'wsp-card-note', key: 'why' },
                matched.length === 0
                  ? '为什么是它：这条人设没有设置适用范围，作为兜底命中。'
                  : `为什么是它：${matched.map((t) => `${t.kind === 'workspace' ? '工作区' : '会话'} ${t.match} ${t.value}`).join('；')}`),
            ],
            body: [
              editing && draft !== null
