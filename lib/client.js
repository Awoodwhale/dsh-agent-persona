...
const since = (timestamp) => {
  ...
  return `${Math.round(seconds / 86400)} 天前`
    ...
    class WorkspacePersonaSection extends React.Component {
      ...
      /** Read on until the end of the conversation (bounded), in one click. */
      async loadAllHistory() {
        ...
        this.onChange({ historyBusy: current.id, error: '' })
        try {
          for (let page = 0; page < 20 && current.done !== true && current.nextOffset !== null; page += 1) {
            const next = this.unwrap(await api.sessionHistory({ id: current.id, offset: current.nextOffset, events: 2000 }))
            current = {
              ...next,
              messages: [...(current.messages ?? []), ...(next?.messages ?? [])],
            ...
            this.onChange({ history: current })
          }
          this.onChange({ historyBusy: undefined, history: current })
        } catch (error) {
          this.onChange({ historyBusy: undefined, error: String((error && error.message) || error) })
        }
      }
          ...
          return h('div', { className: 'wsp-card-body', key: `b${persona.id}` }, [
            ...
            h('div', { className: 'wsp-field-row', key: 'mode' }, [
              ...
              h('span', { className: 'wsp-hint', key: 'h', style: { flex: '1 1 200px', minWidth: '160px' } },
                draft.mode === 'replace'
                  ? '替换：DSH 本来会先说"你是一个由 DeepSeek Harness 驱动的编码智能体"，选这个就不说那句了，改由下面这段人设来当身份。注意：如果某个 Agent 预设自己也写了身份，它不会被覆盖。'
                  : '追加：DSH 先说它自带的身份，然后紧接着说下面这段人设，两句都生效（适合给现有身份补充要求）。'),
            ]),
        ...
        const historyModal = h(Modal, {
          ...
          footer: history === undefined || history.unavailable === true ? null : h('div', { className: 'wsp-actions', key: 'f' }, [
            h('span', { className: 'wsp-caption', key: 'c' }, history.done === true
              ...
              : `已读到 ${history.shown ?? 0} 条（从会话开头顺序读，后面的还没读）`),
            h(Button, {
              key: 'all',
              variant: 'ghost',
              size: 'sm',
              disabled: state.historyBusy !== undefined || history.done === true,
              onClick: () => void this.loadAllHistory(),
            }, '读到最新'),
            h(Button, {
              key: 'more',
              variant: 'outline',
