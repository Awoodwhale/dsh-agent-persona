...
.wsp-warmup { position: absolute; width: 1px; height: 1px; overflow: hidden; opacity: 0; pointer-events: none; }
...
`
    ...
    /**
     * A message that mentions something like `<MyThing>` in prose would otherwise be
     * parsed as an unknown HTML element and fail that's render (the renderer
    ...
    class SafeMarkdown extends React.Component {
      constructor(props) {
        ...
        this.state = { failed: false, reason: undefined, attempt: 0 }
      ...
      componentDidCatch(error) {
        ...
        // A clipped fragment can fail to render while the same message in full renders
        // fine, so ask the owner to fetch the whole text for this bubble.
        // The throw comes from the kit's syntax highlighter while it compiles grammars
        // for this pass (the warm-up samples are constants, so the undefined value lives
        // inside the tokenizer). Rendering the same text again once the highlighter has
        // settled succeeds — verified by hand: expanding the very message that failed
        // renders its headings, lists and links. So retry a few times before giving up.
        if (this.state.attempt < 3 && this.retry === undefined) {
          this.retry = setTimeout(() => {
            this.retry = undefined
            this.setState({ failed: false, attempt: this.state.attempt + 1 })
          }, 500)
        }
        if (typeof this.props.onDegrade === 'function' && this.props.degraded !== true) this.props.onDegrade()
      }
  ...
  return `${Math.round(seconds / 86400)} 天前`
    ...
    class WorkspacePersonaSection extends React.Component {
      ...
      renderBody() {
            ...
            return h('div', { className: mine ? 'wsp-msg wsp-msg-mine' : 'wsp-msg', key: `m${i}` }, [
              ...
              h('div', { className: 'wsp-msg-col', key: 'c' }, [
                ...
                h('div', { className: mine ? 'wsp-bubble wsp-bubble-mine' : 'wsp-bubble', key: 'b' },
                  h(SafeMarkdown, {
                    text: message.text,
                    degraded: message.expanded === true,
                    onDegrade: message.truncated === true && message.expanded !== true && message.at !== undefined
                      ? () => void this.expandMessage(i)
                      : undefined,
                  })),
        ...
        // Warm the kit's syntax highlighter here, inside this boundary: its boot pass
        // walks BOOT_GRAMMAR_WARMUPS and reads `.code` off an entry that can be
        // undefined, so the throw lands in whichever markdown surface renders a code
        // fence first — which was the conversation dialog. Burning it once on a hidden
        // node keeps the failure off screen and leaves the transcript rendering.
        const warmup = h('div', { className: 'wsp-warmup', key: 'warmup', 'aria-hidden': 'true' },
          h(SafeMarkdown, { text: '```bash\nwarm\n```\n\n```js\nwarm\n```\n\n```python\nwarm\n```\n\n```json\nwarm\n```\n\n```diff\nwarm\n```' }))

        return h('div', rootProps, [header, searchRow, hiddenNote, conflictNotice, h('div', { className: 'wsp-cards', key: 'cards' }, cards), status, warmup, historyModal])
      }
    }
