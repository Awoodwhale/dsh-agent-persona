    ...
    class WorkspacePersonaSection extends React.Component {
      ...
      workspaceItems() {
        ...
        return workspaces.map((workspace) => ({
          ...
          label: h('span', { className: 'wsp-opt' }, [
            ...
            workspace.owner === undefined ? null : h('span', { className: 'wsp-opt-owner', key: 'o' }, `已被「${workspace.owner.name}」用`),
      ...
      sessionItems() {
        ...
        const optionLabel = (session) => h('span', { className: 'wsp-opt' }, [
          ...
          session.owner === undefined ? null : h('span', { className: 'wsp-opt-owner', key: 'o' }, `已被「${session.owner.name}」用`),
      ...
      renderBody() {
        ...
        /** Session lookup at renderBody scope (the modal title needs it too). */
        ...
        const historyModal = h(Modal, {
          ...
          : h('div', { className: 'wsp-history' }, (history.messages ?? []).map((message, i) => h('div', {
            ...
            h('span', { className: 'wsp-history-role', key: 'r' }, [
              message.role === 'user' ? '用户' : 'Agent',
              message.truncated === true ? '（太长，已截断）' : null,
            ]),
