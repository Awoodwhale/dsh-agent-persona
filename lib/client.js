...
`
      ...
      render() {
        ...
        if (this.state.failed) {
          return h('div', { className: 'wsp-md-fallback' }, [
            h('div', { className: 'wsp-md-note', key: 'n' }, '这条消息没能按 Markdown 渲染，下面是原文：'),
            h('pre', { className: 'wsp-plain', key: 'p' }, text),
          ])
        }
        // The conversation transcript's own component: same parser, same options as
        // the chat page, so a message looks here exactly as it does there.
        return h(MessageText, { text })
      }
    }
    ...
    const verifyElements = () => {
      const table = {
        Button, DisclosureRow, Input, MessageText, Menu, Modal, Pill, Toast, Tooltip,
        ...
        const cardBody = (persona) => {
          ...
          const tuneBlock = h(DisclosureRow, {
            ...
            state.proposal === '' ? null : h('div', { className: 'wsp-proposal', key: 'prop' }, [
              ...
              h('div', { className: 'wsp-proposal-body', key: 'pb' }, h(MessageText, { text: state.proposal })),
            ]),
          ]) : null)
