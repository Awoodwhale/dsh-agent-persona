...
`
    ...
    /**
     * Turn fenced code blocks into their HTML equivalent. The kit's code-block component
     * drives a syntax highlighter that throws on this content ('Cannot read properties of
     * undefined (reading code)'), and a fence is the only way it can reach it: the
     * renderer's plain HTML route draws the same code without that component. Inline code
     * is left alone, and an unterminated fence still renders as code.
     */
    const fencesToHtml = (text) => {
      if (typeof text !== 'string' || !text.includes('```')) return text
      const out = []
      let body = []
      let open = false
      ...
      if (open) out.push(`<pre><code>${escapeHtmlText(body.join('\n'))}</code></pre>`)
      return out.join('\n')
    }
    ...
    class SafeMarkdown extends React.Component {
      ...
      componentDidCatch(error) {
        ...
        // Retry once the pass is over: this render failed, but nothing about the text is
        // wrong (the same content renders when the same message is opened again), so
        // giving up on the first attempt would pin a plain-text view on a healthy message.
      ...
      render() {
        ...
        return h(MarkdownText, { text: fencesToHtml(escapeUnknownTags(text)) })
      }
    }
