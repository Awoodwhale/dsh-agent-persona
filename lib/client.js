  ...
  return `${Math.round(seconds / 86400)} 天前`
    ...
    class WorkspacePersonaSection extends React.Component {
      ...
      renderBody() {
        ...
        const hiddenNote = view.hidden === undefined || (view.hidden.subagents === 0 && view.hidden.archived === 0 && view.hidden.blank === 0) ? null
          : h('div', { className: 'wsp-hint', key: 'hidden' }, `会话下拉里只列可交互的会话（已隐藏 ${view.hidden.subagents} 个子 agent 会话、${view.hidden.archived} 个已归档会话、${view.hidden.blank} 个空会话）`)
