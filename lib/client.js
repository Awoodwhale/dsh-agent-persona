      ...
      render() {
        ...
        if (loading) {
          ...
          panel = h('div', { className: 'wsp-panel-area', key: 'empty' }, card({
            key: 'c',
            title: '没有命中任何人设',
            note: '这条会话使用 DSH 原生 system prompt。要给它指定人设，去「设置 → Agent 人设」新建一条，并把适用范围指到这条会话或它的工作区。',
          }))
        } else if (tab === 'persona') {
              ...
              h('div', { className: 'wsp-msg-col', key: 'c' }, [
                ...
                message.truncated === true || message.expanded === true
                  ? h('button', {
                    ...
                    h(message.expanded === true ? IconChevronUpOutline14 : IconChevronDownOutline14, { key: 'i' }),
                    h('span', { key: 'l' }, state.historyBusy === `msg:${i}` ? '展开中…' : message.expanded === true ? '收起全文' : '展开全文'),
                  ])
                  : null,
              ]),
            ])
