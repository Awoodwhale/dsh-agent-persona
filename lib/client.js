 ...
 *   - a "命中测试" disclosure asks the HOST which persona a given cwd/session
 *     would get — the answer comes from the same resolver the prompt assembly
 *     uses, so the page can never drift from what actually gets injected.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const KIND_LABEL = { workspace: '工作区目录', sessionId: '会话 ID' }
    const MATCH_LABEL = { exact: '精确', prefix: '前缀', regex: '正则', contains: '包含' }
    const MODES = [
    ...
    const REASON_TEXT = {
      target: '由适用范围命中',
      'catch-all': '兜底人设（没有设置适用范围）',
      none: '没有任何人设命中 → 使用 DSH 原生 system prompt',
    }
...
.wsp-target { display: flex; align-items: center; gap: 6px; min-width: 0; }
.wsp-target-kind { flex: none; width: 96px; }
.wsp-target-match { flex: none; width: 84px; }
.wsp-target-value { flex: 1; min-width: 90px; }
...
.wsp-verdict { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; padding: 9px 10px; border-radius: var(--wsp-radius-sm); background: var(--wsp-surface); font-size: 12px; }
...
`
    ...
    class WorkspacePersonaSection extends React.Component {
      ...
      async load(keepOpen) {
        const api = this.api()
        if (api === undefined) {
          this.onChange({ error: 'remote.workspacePersona 未挂载：宿主插件未加载' })
          return
        }
      ...
      async create() {
        ...
        try {
          ...
          if (created !== undefined) {
            this.open(created)
            this.onChange({ status: '已新建（默认停用）：设置好适用范围后再打开「启用」' })
          }
        } catch (error) {
          this.onChange({ busy: '', error: String((error && error.message) || error) })
        }
      ...
      async move(id, delta) {
        ...
        try {
          ...
          this.onChange({ view, busy: '', status: delta < 0 ? '已上移（优先级提高）' : '已下移（优先级降低）' })
        } catch (error) {
          this.onChange({ busy: '', error: String((error && error.message) || error) })
        }
      }
      ...
      async probe() {
        const api = this.api()
        if (api === undefined) return
        this.onChange({ busy: 'probe', error: '', probe: undefined })
        try {
          const result = this.unwrap(await api.previewMatch({ cwd: this.state.probeCwd, sessionId: this.state.probeSession }))
          this.onChange({ busy: '', probe: result })
        } catch (error) {
          this.onChange({ busy: '', error: String((error && error.message) || error) })
        }
      }
      ...
      renderBody() {
        ...
        const header = h('div', { className: 'wsp-head', key: 'head' }, [
          h('div', { className: 'wsp-head-top', key: 'top' }, [
            ...
            h(Badge, { key: 'b', tone: view.counts.enabled > 0 ? 'on' : undefined }, `${view.counts.total} 个人设 · ${view.counts.enabled} 个启用`),
          ...
          h('p', { className: 'wsp-sub', key: 's' }, [
            '每个人设自己声明适用范围（工作区目录 / 会话 ID × 精确 / 前缀 / 正则 / 包含）。',
            '列表顺序就是优先级：自上而下第一个命中的生效；没填适用范围的人设是兜底；',
            '正文留空的人设一旦命中就表示「这里不要人设」（可用来给兜底开例外）。',
          ]),
          h('div', { className: 'wsp-meta', key: 'm' }, [
            h('span', { key: 'p' }, ['存储 ', h('code', { key: 'c' }, view.storePath)]),
            h('span', { className: 'wsp-sep', key: 's1' }, '·'),
            h('span', { key: 'sec' }, ['section ', h('code', { key: 'c' }, view.sectionName), ` @ order ${view.sectionOrder}`]),
            h('span', { className: 'wsp-sep', key: 's2' }, '·'),
            h('span', { key: 'v' }, ['变量 ', h('code', { key: 'c' }, (view.allowedVariables || []).map((n) => `{{${n}}}`).join(' '))]),
          ]),
        ])

        // ── one collapsed row
        const scopeChips = (persona) => {
          if (persona.catchAll) return [h(Badge, { key: 'ca', tone: 'warn' }, '兜底 · 无适用范围')]
          ...
          }, `${target.kind === 'workspace' ? 'DIR' : 'ID'} ${target.invalid ? '未填写' : describeTarget(target)}`))
        ...
        const cardHead = (persona, index) => {
          ...
          return h('div', {
            ...
            }, persona.enabled ? (persona.text.trim() === '' ? '启用·正文空' : '启用') : '已停用'),
        ...
        const cardBody = (persona) => {
          ...
          const targetRows = targets.map((target, index) => h('div', { className: 'wsp-target', key: `t${index}` }, [
            h('select', {
              key: 'k',
              className: 'wsp-select wsp-target-kind',
              value: target.kind,
              'aria-label': '匹配维度',
              onChange: (event) => this.patchTarget(index, { kind: event.target.value }),
            }, (view.targetKinds || []).map((kind) => h('option', { key: kind, value: kind }, KIND_LABEL[kind] ?? kind))),
            h('select', {
              key: 'm',
            ...
            h('input', {
              ...
              placeholder: target.kind === 'workspace'
                ...
                : (target.match === 'exact' ? 'im-bot-1a2b3c-g3' : target.match === 'regex' ? '^im-bot-.*-g[0-9]+$' : 'im-bot-'),
              onChange: (event) => this.patchTarget(index, { value: event.target.value }),
            }),
            h('button', {
              key: 'x',
              type: 'button',
          ...
          const tuneBlock = h(DisclosureRow, {
            ...
            h('span', { className: 'wsp-hint', key: 'h' }, '模型只产出建议稿，不会自动写入；确认后点「采用」或「追加」，再点保存。'),
            state.proposal === '' ? null : h('div', { className: 'wsp-proposal', key: 'prop' }, [
              h('div', { className: 'wsp-row-head', key: 'ph', style: { padding: 0, minHeight: 'auto' } }, [
                h('span', { className: 'wsp-label', key: 'l' }, '建议稿（未保存）'),
                h('span', { className: 'wsp-spacer', key: 'sp' }),
          ...
          return h('div', { className: 'wsp-card-body', key: `b${persona.id}` }, [
            ...
            h('div', { className: 'wsp-group', key: 'scope' }, [
              h('div', { className: 'wsp-row-head', key: 'gh', style: { padding: 0, minHeight: 'auto' } }, [
                h('span', { className: 'wsp-sect-title', key: 't' }, '适用范围'),
                h('span', { className: 'wsp-spacer', key: 'sp' }),
                h('span', { className: 'wsp-hint', key: 'h' }, '同一人设内多条规则是「或」关系'),
              ]),
              ...targetRows,
            ...
            h('div', { className: 'wsp-field', key: 'body' }, [
              ...
              h('div', { className: 'wsp-editor', key: 'ed' }, state.pane === 'edit'
                ? h('textarea', {
                  ...
                  spellCheck: false,
                  placeholder: '在这里写 Markdown 人设。留空 = 这个人设命中后不使用任何人设（可用作例外）。',
                  value: draft.text,
                  onChange: (event) => this.patchDraft({ text: event.target.value }),
                })
                : h('div', { className: 'wsp-preview' }, draft.text.trim() === ''
                  ? h('span', { className: 'wsp-preview-empty' }, '（正文为空 → 命中后不使用人设）')
                  : h(MarkdownText, { text: draft.text }))),
            ]),
            tuneBlock,
            h('div', { className: 'wsp-foot', key: 'foot' }, [
              h('span', { className: 'wsp-caption', key: 'c' }, `${draft.text.length} 字符 · ⌘/Ctrl+S 保存`),
        ...
        const cards = personas.length === 0
          ? [h('div', { className: 'wsp-empty-card', key: 'empty' }, [
            h('span', { key: 't' }, '还没有任何人设。新建一个人设，设置适用范围（或不设 = 兜底），再启用即可。'),
        ...
        // ── match probe
        const verdict = state.probe === undefined ? null : h('div', { className: 'wsp-verdict', key: 'v' }, [
          h(Dot, { key: 'd', state: state.probe.personaId === null ? 'off' : (state.probe.silenced ? 'empty' : 'on') }),
          state.probe.personaId === null
            ? h('span', { key: 't' }, REASON_TEXT.none)
            : h('span', { key: 't' }, [
              '命中：',
              h('strong', { key: 'n' }, state.probe.name),
              state.probe.silenced ? '（正文为空 → 这里不使用人设）' : `（${state.probe.chars} 字符）`,
              h('span', { className: 'wsp-hint', key: 'r' }, ` · ${REASON_TEXT[state.probe.reason] ?? state.probe.reason}`),
            ]),
          ...(state.probe.matchedBy || []).map((target, i) => h('span', { className: 'wsp-chip', key: `m${i}` }, `${KIND_LABEL[target.kind]} · ${describeTarget(target)}`)),
          state.probe.skipped.length > 0 ? h('span', { className: 'wsp-hint', key: 'sk' }, `（已跳过 ${state.probe.skipped.length} 个停用人设）`) : null,
        ])

        const probeCard = h('div', { className: 'wsp-card', key: 'probe' }, h('div', { className: 'wsp-card-body', style: { paddingTop: '9px' } }, [
          h(DisclosureRow, {
            icon: h(IconCheckOutline14, {}),
            title: '命中测试（让宿主真的算一遍）',
            open: state.testOpen,
            expandable: true,
            onToggle: () => this.onChange({ testOpen: !state.testOpen }),
            expandOnRowClick: true,
            keepContentWhenOpen: true,
          }, state.testOpen ? h('div', { className: 'wsp-tune-fields', style: { paddingTop: '10px' } }, [
            h('div', { className: 'wsp-field-row', key: 'r' }, [
              h('div', { className: 'wsp-field', key: 'cwd', style: { flex: 1 } }, [
                h('span', { className: 'wsp-label', key: 'l' }, '工作目录 cwd'),
                h(Input, {
                  key: 'i',
                  value: state.probeCwd,
                  placeholder: '/Users/you/code/my-project',
                  'aria-label': '测试用 cwd',
                  onChange: (event) => this.onChange({ probeCwd: event.target.value }),
                }),
              ]),
              h('div', { className: 'wsp-field', key: 'sid', style: { flex: 1 } }, [
                h('span', { className: 'wsp-label', key: 'l' }, '会话 ID'),
                h(Input, {
                  key: 'i',
                  value: state.probeSession,
                  placeholder: 'im-bot-1a2b3c-g3',
                  'aria-label': '测试用会话 ID',
                  onChange: (event) => this.onChange({ probeSession: event.target.value }),
                }),
              ]),
              h(Button, { key: 'go', variant: 'outline', size: 'sm', disabled: busy, onClick: () => void this.probe() }, '测试'),
            ]),
            verdict,
            h('span', { className: 'wsp-hint', key: 'h' }, '结果由宿主的解析器给出，和真正注入时用的是同一套规则。'),
          ]) : null),
        ]))
        ...
        return h('div', rootProps, [header, h('div', { className: 'wsp-cards', key: 'cards' }, cards), probeCard, status])
      }
    }
