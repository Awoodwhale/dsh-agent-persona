...
.wsp-prefs-note { font-size: 11px; color: var(--wsp-muted-2); }
    ...
    class WorkspacePersonaSection extends React.Component {
      ...
      async toggleEnabled(persona, checked) {
        ...
        try {
          ...
          this.onChange({ view, busy: '', status: checked ? `已启用「${persona.name}」` : `已停用「${persona.name}」` })
        } catch (error) {
      ...
      renderBody() {
        ...
        const prefsRow = h('div', { className: 'wsp-prefs', key: 'prefs' }, [
          ...
          sidebarAvailable
            ...
            : null,
          h('span', { className: 'wsp-prefs-note', key: 'n' }, sidebarAvailable ? '显示位置在重新打开页面后生效' : '显示位置在重新打开页面后生效（未安装侧边栏插件，因此没有该开关）'),
        ])

        return h('div', rootProps, [header, prefsRow, searchRow, hiddenNote, conflictNotice, h('div', { className: 'wsp-cards', key: 'cards' }, cards), status, historyModal])
      }
    }
