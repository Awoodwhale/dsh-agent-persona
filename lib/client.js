...
.wsp-view { display: flex; flex-direction: column; gap: 24px; padding: 28px 32px 40px; overflow: auto; height: 100%; box-sizing: border-box; max-width: 940px; margin: 0 auto; }
.wsp-view-head { display: flex; align-items: flex-start; gap: 16px; }
...
.wsp-panel { display: flex; flex-direction: column; gap: 12px; padding: 16px 18px; border: 1px solid var(--wsp-line); border-radius: 12px; background: var(--wsp-surface); transition: border-color 150ms ease, background 150ms ease; }
.wsp-panel-head { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.wsp-panel-title { margin: 0; font-size: 14px; font-weight: 600; color: var(--wsp-text); }
.wsp-panel-meta { margin: 0; font-size: 12px; color: var(--wsp-muted); }
.wsp-panel-note { margin: 0; font-size: 12.5px; line-height: 1.7; color: var(--wsp-muted); max-width: 72ch; }
.wsp-grow { flex: 1 1 auto; }
.wsp-chip { display: inline-flex; align-items: center; height: 20px; padding: 0 8px; border: 1px solid var(--wsp-line-soft); border-radius: 999px; font-size: 11px; line-height: 1; color: var(--wsp-muted); background: var(--wsp-surface-2); }
.wsp-chip-on { color: var(--wsp-success); border-color: color-mix(in srgb, var(--wsp-success) 40%, transparent); }
.wsp-chip-warn { color: var(--wsp-warn); border-color: color-mix(in srgb, var(--wsp-warn) 40%, transparent); }
.wsp-prose { font-size: 13px; line-height: 1.75; color: var(--wsp-text); max-width: 74ch; }
.wsp-prose-scroll { max-height: 52vh; overflow: auto; padding-right: 4px; }
...
.wsp-form-text { width: 100%; min-height: 240px; resize: vertical; box-sizing: border-box; padding: 10px 12px; border: 1px solid var(--wsp-line); border-radius: 10px; font-family: var(--dsw-font-markdown-code-font-family, ui-monospace, monospace); font-size: 12.5px; line-height: 1.7; color: var(--wsp-text); background: var(--wsp-surface-2); transition: border-color 150ms ease; }
...
.wsp-alert { display: flex; align-items: center; gap: 8px; padding: 10px 12px; border: 1px solid color-mix(in srgb, var(--wsp-danger, #f85149) 40%, transparent); border-radius: 10px; font-size: 12.5px; color: var(--wsp-text); background: color-mix(in srgb, var(--wsp-danger, #f85149) 8%, transparent); }
...
.wsp-view button, .wsp-view textarea, .wsp-view input, .wsp-view [role="button"] { cursor: pointer; }
.wsp-view textarea, .wsp-view input { cursor: text; }
@media (prefers-reduced-motion: reduce) { .wsp-skel-line { animation: none; } .wsp-panel, .wsp-form-text { transition: none; } }
...
`
    ...
    class PersonaView extends React.Component {
      constructor(props) {
        super(props === undefined || props === null ? {} : props)
        this.state = { loading: true, data: null, error: '', busy: '', draft: null, showPrompt: false, editing: false, copied: false }
      }
      ...
      render() {
        const { data, draft, error, busy, showPrompt, loading, editing, copied } = this.state
        ...
        if (loading) {
          rows.push(h('div', { className: 'wsp-skel', key: 'sk' }, [
          ...
          return h('div', { className: 'wsp-view', 'data-plugin': NS }, rows)
        }

        if (error !== '') {
          rows.push(h('div', { className: 'wsp-alert', role: 'alert', key: 'err' }, [
        ...
        if (persona === null) {
          rows.push(h('section', { className: 'wsp-panel', key: 'empty' }, [
            h('div', { className: 'wsp-panel-head', key: 'h' }, h('h3', { className: 'wsp-panel-title' }, '没有命中任何人设')),
            h('p', { className: 'wsp-panel-note', key: 'n' }, '这条会话使用 DSH 原生 system prompt。要给它指定人设，去「设置 → Agent人设」新建一条并把适用范围指到这里。'),
          ]))
          return h('div', { className: 'wsp-view', 'data-plugin': NS }, rows)
        }

        rows.push(h('section', { className: 'wsp-panel', key: 'persona' }, [
          h('div', { className: 'wsp-panel-head', key: 'h' }, [
            h('h3', { className: 'wsp-panel-title', key: 't' }, persona.name),
            h('span', { className: persona.enabled ? 'wsp-chip wsp-chip-on' : 'wsp-chip', key: 'm' },
              persona.mode === 'replace' ? '替换' : '追加'),
            h('span', { className: persona.enabled ? 'wsp-chip wsp-chip-on' : 'wsp-chip', key: 'e' },
              persona.enabled ? '在用' : '已停用'),
            h('span', { className: 'wsp-grow', key: 'g' }),
            dirty ? h('span', { className: 'wsp-chip wsp-chip-warn', key: 'd' }, '有未保存的修改') : null,
            h(Button, {
              key: 'edit',
          ...
          data?.matchedBy !== undefined && data.matchedBy.length > 0
            ? h('p', { className: 'wsp-panel-meta', key: 'm' }, `命中依据：${data.matchedBy.map((t) => `${t.kind === 'workspace' ? '工作区' : '会话'} ${t.match} ${t.value}`).join('；')}`)
            : null,
          editing && draft !== null
            ? h('div', { className: 'wsp-form', key: 'form' }, [
              ...
              h('div', { className: 'wsp-form-actions', key: 'a' }, [
                h('span', { className: 'wsp-panel-meta', key: 'c' }, `${draft.text.length} 字 · 保存后下一个请求生效`),
                h('span', { className: 'wsp-grow', key: 'g' }),
        ...
        rows.push(h('section', { className: 'wsp-panel', key: 'prompt' }, [
          h('div', { className: 'wsp-panel-head', key: 'h' }, [
            h('h3', { className: 'wsp-panel-title', key: 't' }, '完整 system prompt'),
            h('span', { className: 'wsp-panel-meta', key: 'c' }, prompt === null ? '还没有记录' : `${prompt.length} 字 · 最近一次实际发送`),
            h('span', { className: 'wsp-grow', key: 'g' }),
            prompt === null ? null : h(Button, { key: 'copy', variant: 'ghost', size: 'sm', onClick: () => void this.copyPrompt() }, copied ? '已复制' : '复制'),
            prompt === null ? null : h(Button, { key: 'toggle', variant: 'outline', size: 'sm', onClick: () => this.setState({ showPrompt: !showPrompt }) }, showPrompt ? '收起' : '展开'),
          ]),
          prompt === null
            ? h('p', { className: 'wsp-panel-note', key: 'n' }, '这条会话还没有发起过请求，因此没有 system prompt 记录。')
            : showPrompt
