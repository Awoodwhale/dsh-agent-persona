...
.wsp-tabs { display: flex; gap: 6px; max-width: 1040px; width: 100%; margin: 0 auto; }
.wsp-tab { padding: 6px 16px; border: 1px solid var(--wsp-line); border-radius: 999px; background: var(--wsp-surface-2); font: inherit; font-size: 13px; color: var(--wsp-muted); cursor: pointer; transition: color 150ms ease, background 150ms ease, border-color 150ms ease; }
...
.wsp-tab-on { border-color: color-mix(in srgb, var(--wsp-accent) 55%, transparent); background: color-mix(in srgb, var(--wsp-accent) 16%, var(--wsp-surface-2)); color: var(--wsp-accent); font-weight: 600; }
.wsp-panel-area { max-width: 1040px; width: 100%; margin: 0 auto; display: flex; flex-direction: column; gap: 16px; }
.wsp-card-panel { display: flex; flex-direction: column; gap: 14px; padding: 0 20px 20px; border: 1px solid var(--wsp-line-strong); border-radius: 14px; background: var(--wsp-surface-2); box-shadow: var(--wsp-card-shadow); overflow: hidden; transition: box-shadow 200ms ease, border-color 200ms ease; }
.wsp-card-panel:hover { border-color: color-mix(in srgb, var(--wsp-accent) 30%, var(--wsp-line-strong)); box-shadow: 0 6px 22px rgba(0, 0, 0, 0.16); }
...
@media (max-width: 640px) {
  ...
  .wsp-tabs { overflow-x: auto; padding-bottom: 2px; }
  .wsp-tab { flex: none; padding: 6px 13px; }
      ...
      render() {
        ...
        if (loading) {
          ...
          panel = h('div', { className: 'wsp-panel-area', key: 'persona' }, card({
            key: 'c',
            title: persona.name,
            meta: `${persona.text.length} 字`,
            actions: [
              h(Button, { key: 'edit', variant: editing ? 'outline' : 'outline', size: 'sm', onClick: () => this.setState({ editing: !editing }) },
                editing ? '取消编辑' : '编辑正文'),
            ],
            extra: [
              h('div', { className: 'wsp-tags', key: 'tags' }, [
                h('span', { className: persona.enabled ? 'wsp-chip wsp-chip-on' : 'wsp-chip', key: 'e' }, persona.enabled ? '在用' : '已停用'),
                h('span', { className: 'wsp-chip', key: 'm' }, persona.mode === 'replace' ? '替换 DSH 身份' : '追加在 DSH 身份之后'),
