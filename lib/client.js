...
.wsp-panel-area { display: flex; flex-direction: column; gap: 16px; animation: wsp-enter 190ms cubic-bezier(0.2, 0, 0, 1) both; }
...
/* Cross-fade for a tab switch, driven by the View Transitions API. */
.wsp-panel-area { view-transition-name: wsp-panel; }
::view-transition-old(wsp-panel) { animation: wsp-fade-out 200ms cubic-bezier(0.2, 0, 0, 1) both; }
::view-transition-new(wsp-panel) { animation: wsp-fade-in 200ms cubic-bezier(0.2, 0, 0, 1) both; }
@keyframes wsp-fade-out { from { opacity: 1; transform: none; } to { opacity: 0; transform: translateY(-3px); } }
@keyframes wsp-fade-in { from { opacity: 0; transform: translateY(5px); } to { opacity: 1; transform: none; } }
...
@media (prefers-reduced-motion: reduce) {
  ...
  ::view-transition-old(wsp-panel), ::view-transition-new(wsp-panel) { animation: none; }
}
      ...
      render() {
        ...
        const tabs = h('div', { className: 'wsp-tabs', key: 'tabs', role: 'tablist' }, TABS.map((item) => h('button', {
          ...
          onClick: () => {
            ...
            const apply = () => this.setState({ tab: item.id })
            // A real cross-fade: the browser snapshots the old panel, fades it out while the new
            // one fades in, and nothing reflows. Products that feel calm do it this way; without
            // the API (or with reduced motion) the state change is simply applied.
            if (typeof document.startViewTransition === 'function' && !reduce) {
              document.startViewTransition(() => new Promise((done) => {
                apply()
