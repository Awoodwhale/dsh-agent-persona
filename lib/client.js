...
.wsp-view { display: flex; flex-direction: column; gap: 12px; padding: 14px 16px; overflow: auto; height: 100%; box-sizing: border-box; }
.wsp-view-head { display: flex; align-items: baseline; gap: 10px; }
.wsp-view-title { margin: 0; font-size: 15px; font-weight: 600; color: var(--wsp-text); }
.wsp-view-sub { font-size: 12px; color: var(--wsp-muted); }
.wsp-view-note { font-size: 12px; color: var(--wsp-muted); }
.wsp-view-error { font-size: 12px; color: var(--wsp-danger, #f85149); }
.wsp-view-edit { display: flex; flex-direction: column; gap: 8px; border: 1px solid var(--wsp-line); border-radius: 10px; padding: 10px 12px; }
.wsp-view-row { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.wsp-view-label { font-size: 12px; color: var(--wsp-muted); }
.wsp-view-text { width: 100%; min-height: 220px; resize: vertical; box-sizing: border-box; font-family: var(--dsw-font-markdown-code-font-family, ui-monospace, monospace); font-size: 12px; line-height: 1.6; color: var(--wsp-text); background: var(--wsp-surface); border: 1px solid var(--wsp-line); border-radius: 8px; padding: 8px 10px; }
.wsp-view-section { display: flex; flex-direction: column; gap: 6px; border: 1px solid var(--wsp-line); border-radius: 10px; padding: 10px 12px; }
.wsp-view-sechead { display: flex; align-items: center; gap: 8px; font-size: 12px; color: var(--wsp-muted); }
.wsp-view-render { font-size: 12.5px; line-height: 1.7; color: var(--wsp-text); }
.wsp-view-prompt { max-height: 46vh; overflow: auto; }
...
`
    ...
    class PersonaView extends React.Component {
      constructor(props) {
        super(props === undefined || props === null ? {} : props)
        this.state = { loading: true, data: null, error: '', busy: '', draft: null, showPrompt: false }
      }
      ...
      render() {
        const { data, draft, error, busy, showPrompt, loading } = this.state
        const persona = data?.persona ?? null
        const rows = []
        rows.push(h('div', { className: 'wsp-view-head', key: 'head' }, [
          h('h3', { className: 'wsp-view-title', key: 't' }, '人设'),
          h('span', { className: 'wsp-view-sub', key: 's' }, persona === null
            ? '这个会话没有命中任何人设，使用 DSH 原生 prompt'
            : `${persona.name} · ${persona.mode === 'replace' ? '替换' : '追加'} · ${persona.enabled ? '在用' : '已停用'}${draft !== null && (draft.text !== persona.text || draft.name !== persona.name) ? ' · 有未保存的修改' : ''}`),
          h(Button, { key: 'reload', variant: 'ghost', size: 'sm', disabled: busy !== '', onClick: () => void this.load() }, busy === 'load' ? '读取中…' : '重新读取'),
        ]))
        if (loading) rows.push(h('div', { className: 'wsp-view-note', key: 'l' }, '正在读取…'))
        if (error !== '') rows.push(h('div', { className: 'wsp-view-error', key: 'e' }, error))
        if (data !== null && data.matchedBy !== undefined && data.matchedBy.length > 0) {
          rows.push(h('div', { className: 'wsp-view-note', key: 'm' },
            `命中依据：${data.matchedBy.map((t) => `${t.kind === 'workspace' ? '工作区' : '会话'} ${t.match} ${t.value}`).join('；')}`))
        }
        if (persona !== null && draft !== null) {
          rows.push(h('div', { className: 'wsp-view-edit', key: 'edit' }, [
            h('div', { className: 'wsp-view-row', key: 'r1' }, [
              h('span', { className: 'wsp-view-label', key: 'l' }, '名称'),
              h(Input, { key: 'n', value: draft.name, onChange: (event) => this.patch({ name: event.target.value }) }),
            ...
            h('textarea', {
              key: 'text',
              className: 'wsp-view-text',
              value: draft.text,
              spellCheck: false,
              onChange: (event) => this.patch({ text: event.target.value }),
            }),
            h('div', { className: 'wsp-view-row', key: 'r2' }, [
              h('span', { className: 'wsp-caption', key: 'c' }, `${draft.text.length} 字`),
              h('span', { className: 'wsp-spacer', key: 'sp' }),
              h(Button, {
                key: 'save',
                variant: 'primary',
                size: 'sm',
                disabled: busy !== '' || (draft.text === persona.text && draft.name === persona.name && draft.mode === persona.mode),
                onClick: () => void this.save(),
              }, busy === 'save' ? '保存中…' : '保存（下一个请求生效）'),
            ]),
          ]))
        }
        if (persona !== null && draft !== null) {
          rows.push(h('div', { className: 'wsp-view-section', key: 'p' }, [
            h('div', { className: 'wsp-view-sechead', key: 'h' }, [
              h('span', { key: 't' }, '人设正文（渲染效果）'),
            ]),
            h('div', { className: 'wsp-view-render', key: 'r' }, h(SafeMarkdown, { text: draft.text })),
          ]))
        }
        if (data?.prompt !== null && data?.prompt !== undefined) {
          rows.push(h('div', { className: 'wsp-view-section', key: 's' }, [
            h('div', { className: 'wsp-view-sechead', key: 'h' }, [
              h('span', { key: 't' }, `这个会话最近一次实际发送的完整 system prompt（${data.prompt.length} 字）`),
              h('span', { className: 'wsp-spacer', key: 'sp' }),
              h(Button, { key: 'toggle', variant: 'ghost', size: 'sm', onClick: () => this.setState({ showPrompt: !showPrompt }) }, showPrompt ? '收起' : '展开'),
            ]),
            showPrompt ? h('div', { className: 'wsp-view-render wsp-view-prompt', key: 'r' }, h(SafeMarkdown, { text: data.prompt })) : null,
          ]))
        } else if (data !== null) {
          rows.push(h('div', { className: 'wsp-view-note', key: 'np' }, '这个会话还没有可读的 system prompt 记录（还没有发起过请求）'))
        }
