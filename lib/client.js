...
/* The counts and the store path live in .wsp-meta; the tab's own note already says what this
   panel is for, so the whole line stays out of the tab. */
.wsp-panel-area .wsp-title, .wsp-panel-area .wsp-sub, .wsp-panel-area .wsp-meta { display: none; }
...
.wsp-shell > .wsp-view-head, .wsp-shell > .wsp-tabs { animation: wsp-enter 160ms cubic-bezier(0.2, 0, 0, 1) both; }
@keyframes wsp-enter { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }
/* Fade-through on a tab switch, driven by two states instead of a browser API. */
.wsp-panel-area { transition: opacity 130ms cubic-bezier(0.2, 0, 0, 1), transform 130ms cubic-bezier(0.2, 0, 0, 1); }
.wsp-panel-out { opacity: 0; transform: translateY(-3px); }
.wsp-panel-in { opacity: 0; transform: translateY(5px); }
...
`
    ...
    class PersonaView extends React.Component {
      constructor(props) {
        super(props === undefined || props === null ? {} : props)
        this.state = { loading: true, data: null, error: '', busy: '', draft: null, editing: false, copied: false, tab: 'persona', promptView: 'render', tabFading: false, tabEntering: false }
      }
      ...
      render() {
        const { data, draft, error, busy, loading, editing, copied, tab, promptView, tabFading, tabEntering } = this.state
        ...
        const panelClass = (key) => [
          'wsp-panel-area',
          tabFading ? 'wsp-panel-out' : '',
          tabEntering ? 'wsp-panel-in' : '',
        ].filter(Boolean).join(' ')
        ...
        const tabs = h('div', { className: 'wsp-tabs', key: 'tabs', role: 'tablist' }, TABS.map((item) => h('button', {
          ...
          onClick: () => void this.switchTab(item.id),
        }, item.label)))
        ...
        let panel
        if (loading) {
          panel = h('div', { className: panelClass('sk'), key: 'sk' }, h('div', { className: 'wsp-skel' }, [
          ...
          panel = h('div', { className: panelClass('empty'), key: 'empty' }, card({
            key: 'c',
            title: '没有命中任何人设',
            note: '这条会话使用 DSH 原生 system prompt。要给它指定人设，去「设置 → Agent人设」新建一条，并把适用范围指到这条会话或它的工作区。',
          }))
        } else if (tab === 'persona') {
          panel = h('div', { className: panelClass('persona'), key: 'persona' }, card({
            key: 'c',
            title: persona.name,
            meta: undefined,
            tags: [
          ...
          panel = h('div', { className: panelClass('manage'), key: 'manage' },
            h(WorkspacePersonaSection, { key: 'settings', api: this.props?.api ?? capturedApi }))
        } else {
          panel = h('div', { className: panelClass('prompt'), key: 'prompt' }, card({
            key: 'c',
            title: 'System Prompt',
...
const since = (timestamp) => {
  ...
  return `${Math.round(seconds / 86400)} 天前`
    ...
    class WorkspacePersonaSection extends React.Component {
       ...
       */
      /**
       * Switch panels with a fade-through: the current panel fades out (130ms), the next one is
       * mounted hidden and fades in (130ms). Written by hand rather than with the browser's
       * View Transitions API — that one snapshots the DOM inside a callback whose promise must
       * settle in time, and React's asynchronous rendering made it abort with a TimeoutError and
       * freeze the page.
       */
      switchTab(id) {
        if (id === this.state.tab) return
        const reduce = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false
        const scroller = this.scroller
        if (reduce) {
          this.setState({ tab: id, tabFading: false, tabEntering: false })
          scroller?.scrollTo?.({ top: 0, behavior: 'auto' })
          return
        }
        clearTimeout(this.fadeTimer)
        clearTimeout(this.enterTimer)
        this.setState({ tabFading: true })
        this.fadeTimer = setTimeout(() => {
          this.setState({ tab: id, tabFading: false, tabEntering: true })
          requestAnimationFrame(() => {
            this.enterTimer = setTimeout(() => this.setState({ tabEntering: false }), 16)
          })
