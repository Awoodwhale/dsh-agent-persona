...
/**
 * A session's title, read from DSH's projection cache — the same text the Web
 * sidebar shows. The service shape is probed defensively (a record view, a raw
 * record, or the projection file on disk) because this is a nice-to-have.
 */
const projectionTitles = new Map()

const titleFromProjections = async (ctx, id) => {
  if (projectionTitles.has(id)) return projectionTitles.get(id)
  const resolved = await readProjectionTitle(ctx, id)
  if (projectionTitles.size > 400) projectionTitles.clear()
  projectionTitles.set(id, resolved)
  return resolved
}

const readProjectionTitle = async (ctx, id) => {
  const pick = (record) => {
    for (const candidate of [record?.rows?.title, record?.title]) {
      const value = typeof candidate === 'string' ? candidate : candidate?.val
      if (typeof value === 'string' && value !== '') return value
    }
    return undefined
  }
  // The cache service is the intended fast path, but in this DSH version
  // `recordFor(id)` needs an identity witness (`expected.formatVersion`) that a
  // caller cannot reconstruct from a listed header, so it throws and the
  // projection file below is what actually answers. Both are kept: the service
  // call costs one attempt per session per process (results are cached).
  try {
    const cache = ctx.get('sessionProjectionCache')
    const record = cache?.recordFor?.(id)
    if (record !== undefined) {
      const fromView = pick(cache?.viewRecord?.(record, ['title'])) ?? pick(record)
  ...
  try {
    ...
    return pick(parsed?.record) ?? pick(parsed)
  } catch {
    return undefined
  }
}

/** Titles derived from a session's first user message, kept for the process. */
const derivedTitles = new Map()

/**
 * Fall back to the first user message when a session has no title yet, so the
 * picker still says *something* recognisable. Cached: at most one read per
 * session per process.
 */
const titleFromFirstMessage = async (ctx, id) => {
  if (derivedTitles.has(id)) return derivedTitles.get(id)
  let title
  try {
    const history = await collectSessionHistory(ctx, id, 6)
    ...
    if (typeof first?.text === 'string' && first.text !== '') {
      ...
      title = flat.length > 48 ? `${flat.slice(0, 48)}…` : flat
    }
  } catch {
    title = undefined
  }
  if (derivedTitles.size > 400) derivedTitles.clear()
...
export const collectTargets = async (ctx, { personas = [], warn = () => {} } = {}) => {
  ...
  const ownerOf = (kind, value) => {
    for (const persona of personas) {
      ...
      if ((persona.targets ?? []).some((target) => target.kind === kind && target.value === value)) {
  ...
  // real machine, up to ~850 KB), and parsing one costs ~1 ms. Only the newest
  // slice is read inline — enough to label the flat "recent sessions" list and
  // the first directory groups — reads are async so the host loop is never
  // blocked, and every answer (including "no title") is cached for the process.
  let projectionBudget = 16
  let budget = 12
  for (const session of newest) {
    if (session.title === undefined && projectionBudget > 0) {
      projectionBudget -= 1
      const fromProjection = await titleFromProjections(ctx, session.id)
      if (fromProjection !== undefined) session.title = fromProjection
    }
    if (session.title !== undefined || budget <= 0) continue
    budget -= 1
    const derived = await titleFromFirstMessage(ctx, session.id)
    if (derived !== undefined) session.title = derived
  }
  return { workspaces, sessions: newest }
}
...
export const applyPersonaMode = (assembly, { mode, deploymentPrefix }) => {
  ...
  const index = sections.findIndex((entry) => entry?.name === PERSONA_PREFIX_SECTION)
  if (index === -1) return assembly
  const current = typeof sections[index].text === 'string' ? sections[index].text : ''
  if (deploymentPrefix !== undefined && current !== deploymentPrefix) return assembly
  const next = sections.slice()
  next[index] = { ...next[index], text: '' }
  return { ...assembly, sections: next }
}
...
/**
 * Read the tail of one session's conversation, so the settings page can show
 * what a session was about before a persona is pointed at it.
 *
 * Storage is addressed through per-session handles: `open()` gives the handle,
 * while a cold session's events only come out of `handle.read()`. Every probe is
 * defensive — an unavailable reader reports `unavailable` instead of throwing.
 * @param ctx - host context.
 * @param id - session id.
 * @param limit - how many messages to keep (1..20, default 6).
 * @returns `{ id, available, unavailable, cwd, createdAt, total, messages }`.
 */
export const collectSessionHistory = async (ctx, id, limit) => {
  const sessionId = typeof id === 'string' ? id : ''
  if (sessionId === '') return { id: '', available: false, unavailable: true, messages: [] }
  const keep = Number.isFinite(limit) ? Math.max(1, Math.min(20, Math.trunc(limit))) : 6
  let handle
  let persistence
  try {
  ...
  for (const name of ['open', 'openSession', 'handle', 'read', 'load', 'get']) {
    ...
    try {
      handle = await reader.call(persistence, sessionId)
    } catch {
      handle = undefined
    }
  ...
  if (handle === undefined) {
    return { id: sessionId, available: false, unavailable: true, messages: [] }
  }

  let events = Array.isArray(handle.events) ? handle.events : []
  if (events.length === 0 && typeof handle.read === 'function') {
    try {
      const page = await handle.read(0, 400)
      if (Array.isArray(page?.events)) events = page.events
      else if (Array.isArray(page)) events = page
    } catch {
      /* leave events empty; the page says so */
    }
  }
  const messages = []
  for (const event of events) {
    ...
    if (text !== '') messages.push({ role: kind === 'user/message' ? 'user' : 'assistant', text: text.slice(0, 800) })
  }
...
markRemote(WorkspacePersonaService, [
  'listPersonas', 'savePersona', 'deletePersona', 'movePersona', 'duplicatePersona', 'listTargets', 'sessionHistory', 'tunePersona', 'listModels',
])
...
export function apply(ctx, config = {}) {
  ...
  ctx.on('system-prompt/assemble', (assembly, context, next) => {
    try {
      ...
      if (persona === undefined || persona.mode !== 'replace') return next()
      ...
      return applyPersonaMode(assembly, { mode: 'replace', deploymentPrefix: own })
    } catch (error) {
  ...
  const runtime = {
    ...
    save(input) {
      ...
      if (next.id === undefined) {
        persona = { id: newId(), name: next.name, enabled: next.enabled, text: next.text, mode: next.mode, targets: next.targets }
    ...
    async history(input) {
      return collectSessionHistory(ctx, input?.id, input?.limit)
    },
