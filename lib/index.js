/**
 * workspace-persona — host half (v2: many personas, each with its own scope).
 *
 * Model
 * -----
 ...
 *   3. a persona with NO targets is a catch-all: it matches anything, so it
 ...
 *
 * Storage lives in this plugin's own directory — `$DSH_HOME/dsh-workspace-persona/`
 ...
 *   this deployment; when unset the plugin uses the host's own
 *   `agentDefaultModel` selection, and the settings page lists both.
...
export const name = 'workspace-persona'
export const inject = ['systemPrompt']

const DSH_HOME = process.env.DSH_HOME ?? join(homedir(), '.dsh')
/** Everything this plugin writes lives under one directory of its own. */
const STATE_DIR = join(DSH_HOME, 'dsh-workspace-persona')
...
const LEGACY_STORE_FILE = join(DSH_HOME, 'workspace-personas.json')
const LEGACY_HEARTBEAT_FILE = join(DSH_HOME, 'workspace-persona.state.json')
const SECTION_NAME = 'workspace-persona'
 ...
 * persona, and the v1 default becomes a target-less catch-all.
 ...
 */
export const migrateStore = (parsed, warn = () => {}) => {
  if (parsed !== null && typeof parsed === 'object' && parsed.version === STORE_VERSION && Array.isArray(parsed.personas)) {
    return { store: { version: STORE_VERSION, personas: parsed.personas.map(normalizePersona) }, report: undefined }
  }
  ...
  for (const entry of workspaces) {
    ...
    const prefixes = Array.isArray(entry?.sessionPrefixes) ? entry.sessionPrefixes.filter((p) => typeof p === 'string' && p !== '') : []
    const meaningful = text.trim() !== '' || entry?.enabled === true || prefixes.length > 0
    const label = (typeof entry?.title === 'string' && entry.title !== '' ? entry.title : String(entry?.path ?? '').split('/').filter(Boolean).pop()) || '工作区'
    if (!meaningful) {
      dropped.push(`${label}（v1 中已停用且正文为空）`)
      continue
    }
    const targets = []
    if (typeof entry?.path === 'string' && entry.path !== '') targets.push({ kind: 'workspace', match: 'exact', value: resolve(entry.path) })
    for (const prefix of prefixes) targets.push({ kind: 'sessionId', match: 'prefix', value: prefix })
    personas.push(normalizePersona({ name: label, enabled: entry?.enabled !== false, text, targets }))
  }

  const legacyDefault = parsed?.default
  if (legacyDefault !== undefined && (legacyDefault?.enabled === true || (typeof legacyDefault?.text === 'string' && legacyDefault.text.trim() !== ''))) {
    personas.push(normalizePersona({ name: '默认人设（v1 兜底）', enabled: legacyDefault?.enabled === true, text: legacyDefault?.text ?? '', targets: [] }))
  }

  if (workspaces.length > 0 || legacyDefault !== undefined) {
    warn(`migrated the v1 store to v${STORE_VERSION}: ${personas.length} persona(s)${dropped.length === 0 ? '' : `; dropped ${dropped.join('、')}`}`)
  }
  return { store: { version: STORE_VERSION, personas }, report: { migrated: personas.length, dropped } }
}
...
const loadStore = (storePath, warn) => {
  ...
  return migrateStore(parsed, warn).store
}
...
/**
 * One-time move of the pre-0.1.1 layout: `$DSH_HOME/workspace-personas.json`
 * (and its heartbeat) become `<stateDir>/personas.json` / `state.json`, with the
 * old data file kept beside the new one as a `.legacy.bak.json`.
 * @returns the path to read from (never throws; a failure just leaves the legacy file in place).
 */
const adoptLegacyLayout = (storePath, warn) => {
  if (storePath !== STORE_FILE) return storePath
  try {
    // Nothing of ours at the legacy location: nothing to do.
    readFileSync(LEGACY_STORE_FILE, 'utf8')
  } catch {
    return storePath
  }
  try {
    const current = loadStore(storePath, warn)
    if (current.personas.length > 0) return storePath // new file is already in use
    const legacy = loadStore(LEGACY_STORE_FILE, warn)
    persist(legacy, storePath)
    mkdirSync(dirname(storePath), { recursive: true })
    renameSync(LEGACY_STORE_FILE, join(STATE_DIR, 'personas.legacy.bak.json'))
    try {
      unlinkSync(LEGACY_HEARTBEAT_FILE)
    } catch {
      /* the old heartbeat is derived state; absent is fine */
    }
    warn(`moved the old store to ${storePath} and kept a backup at ${join(STATE_DIR, 'personas.legacy.bak.json')}`)
  } catch (error) {
    warn(`could not adopt the legacy store at ${LEGACY_STORE_FILE}; leaving it in place`, error)
  }
  return storePath
}

/**
 * Neutralise `{{…}}` groups the prompt renderer would reject. `renderPrompt()`
 * THROWS on an unknown variable or a malformed reference, which would fail every
 * request, so the text is first stripped of all double braces and only then are
 * the supported variables re-injected.
 */
...
export const matchTarget = (target, subject) => {
  ...
  // makes a plain substring like `infoflow` usable on a cwd.
  switch (target.match) {
    case 'exact':
...
export const resolvePersona = (personas, subject) => {
  for (const persona of personas ?? []) {
    ...
    if (persona.targets.length === 0) return { persona, reason: 'catch-all', matchedBy: [] }
    const matchedBy = matchingTargets(persona, subject)
    if (matchedBy.length > 0) return { persona, reason: 'target', matchedBy }
  }
  return { persona: undefined, reason: 'none', matchedBy: [] }
}
...
export const personaTextFor = (personas, agent, warn = () => {}) => {
  // An agent-less assembly has no cwd/session, so only catch-all personas can
...
/** `workspacePersona` — the namespace the settings page talks to. */
class WorkspacePersonaService extends TypertRemoteService {
  constructor(ctx, runtime) {
    super(ctx, 'workspacePersona')
  ...
  async previewMatch(input) {
    try {
      return this.runtime.preview(input)
    } catch (error) {
      return { error: `previewMatch failed: ${String((error && error.stack) || error)}` }
    }
  }
...
markRemote(WorkspacePersonaService, [
  'listPersonas', 'savePersona', 'deletePersona', 'movePersona', 'duplicatePersona', 'previewMatch', 'tunePersona', 'listModels',
])
...
export function apply(ctx, config = {}) {
  ...
  const warn = (message, error) =>
    logger.warn?.(`[workspace-persona] ${message}${error === undefined ? '' : `: ${String(error)}`}`)
  const info = (message) => logger.info?.(`[workspace-persona] ${message}`)

  // Where this instance keeps its data. `storePath` in the row config overrides
  // the default (`$DSH_HOME/dsh-workspace-persona/personas.json`).
  const storePath = typeof config?.storePath === 'string' && config.storePath !== ''
    ? resolve(config.storePath)
    : STORE_FILE
  adoptLegacyLayout(storePath, warn)
  ...
  const view = () => {
    ...
    const personas = current.personas.map((persona, index) => ({
      ...
      catchAll: persona.targets.length === 0,
    }))
    return {
  ...
  const runtime = {
    ...
    save(input) {
      ...
      if (next.id === undefined) {
        current.personas.push({ id: newId(), name: next.name, enabled: next.enabled, text: next.text, targets: next.targets })
        info(`created persona "${next.name}" (${current.personas.length} total)`)
      } else {
        const existing = current.personas.find((persona) => persona.id === next.id)
        if (existing === undefined) throw new Error('人设不存在（可能已被删除）')
    ...
    preview(input) {
      const current = reload()
      const subject = {
        cwd: typeof input?.cwd === 'string' && input.cwd.trim() !== '' ? resolve(input.cwd.trim()) : undefined,
        sessionId: typeof input?.sessionId === 'string' && input.sessionId.trim() !== '' ? input.sessionId.trim() : undefined,
      }
      const picked = resolvePersona(current.personas, subject)
      return {
        subject,
        personaId: picked.persona?.id ?? null,
        name: picked.persona?.name ?? null,
        reason: picked.reason,
        silenced: picked.persona !== undefined && picked.persona.text.trim() === '',
        chars: picked.persona?.text.length ?? 0,
        matchedBy: picked.matchedBy.map((target) => ({ ...target })),
        skipped: current.personas
          .map((persona, index) => ({ index, name: persona.name, enabled: persona.enabled, targets: persona.targets.length }))
    ...
    async tune(input) {
      ...
      if (route === undefined) {
        throw new Error('没有可用模型：请在设置页选择模型，或在 profile 里给 workspace-persona 这一行配置 tuneProvider / tuneModel')
      }
  ...
  try {
    ...
    info('remote namespace "workspacePersona" registered (v2: multi-persona)')
  } catch (error) {
    warn('could not register the workspacePersona service', error)
  }
