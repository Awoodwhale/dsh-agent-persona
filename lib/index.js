...
export const collectTargets = async (ctx, { personas = [], warn = () => {} } = {}) => {
  ...
  // Projection files can be large (they carry context timelines), so only the
  // newest slice is consulted, and every answer — including "no title" — is kept
  // for the process.
  let projectionBudget = 40
  let budget = 12
  for (const session of newest) {
    if (session.title === undefined && projectionBudget > 0) {
      projectionBudget -= 1
      const fromProjection = titleFromProjections(ctx, session.id)
