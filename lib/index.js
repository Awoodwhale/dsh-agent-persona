 ...
 * @param options.maxChars - per-message text cap (default 12000).
...
export const collectSessionHistory = async (ctx, id, options = {}) => {
  ...
  const maxChars = Number.isFinite(options?.maxChars) ? Math.max(200, Math.min(40000, Math.trunc(options.maxChars))) : 12000
