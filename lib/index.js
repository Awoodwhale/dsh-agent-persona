...
export function apply(ctx, config = {}) {
  ...
  const runtime = {
    ...
    /**
     * A short read of one session's conversation, so the settings page can show
     * what a session was about before a persona is pointed at it.
     */
    async history(input) {
      const id = typeof input?.id === 'string' ? input.id : ''
      if (id === '') return { error: 'sessionHistory needs a session id' }
      const limit = Number.isFinite(input?.limit) ? Math.max(1, Math.min(20, Math.trunc(input.limit))) : 6
      let handle
      const persistence = ctx.get('sessionPersistence')
      for (const name of ['handle', 'open', 'openSession', 'read', 'load', 'get']) {
        const reader = persistence?.[name]
        if (typeof reader !== 'function') continue
        try {
          handle = await reader.call(persistence, id)
        } catch {
          handle = undefined
        }
      ...
      const events = Array.isArray(handle?.events) ? handle.events : []
      const messages = []
      ...
      return {
        id,
        available: handle !== undefined && messages.length > 0,
        cwd: typeof header.cwd === 'string' ? header.cwd : null,
        createdAt: typeof header.createdAt === 'number' ? header.createdAt : null,
