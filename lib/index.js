 ...
 * @param options.events - how many events to read in this window (default 600, max 2000).
 * @param options.maxChars - per-message text cap (default 4000).
...
export const collectSessionHistory = async (ctx, id, options = {}) => {
  ...
  const window = Number.isFinite(options?.events) ? Math.max(1, Math.min(2000, Math.trunc(options.events))) : 600
  const maxChars = Number.isFinite(options?.maxChars) ? Math.max(200, Math.min(20000, Math.trunc(options.maxChars))) : 4000
  ...
  for (const event of events) {
    ...
    messages.push({
      ...
      text: clipped ? `${text.slice(0, maxChars)}…` : text,
...
markRemote(WorkspacePersonaService, [
  'listPersonas', 'savePersona', 'deletePersona', 'movePersona', 'duplicatePersona', 'reorderPersona', 'listTargets', 'sessionHistory', 'tunePersona', 'listModels',
])
...
export function apply(ctx, config = {}) {
  ...
  const runtime = {
    ...
    async history(input) {
      return collectSessionHistory(ctx, input?.id, {
        offset: input?.offset,
        events: input?.events,
