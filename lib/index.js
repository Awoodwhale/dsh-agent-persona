...
/** The host methods the settings page and the persona view reach over Connection RPC. */
class WorkspacePersonaService {
...
export function apply(ctx, config = {}) {
  ...
  let service
  try {
    service = new WorkspacePersonaService(ctx, runtime)
    info('persona service ready (v2: multi-persona)')

    // The browser half reaches these methods over Connection RPC: a channel string, an
    // endpoint name and a JSON envelope — no shared module identity required.
    ctx.effect(() => {
      const connection = ctx.get('connection')
      if (connection === undefined || connection.rpc === undefined) {
        warn('no connection service; the settings page and the persona view cannot reach this host')
        return () => {}
      }
      const dispatch = async (endpoint, payload) => {
        ...
        try {
          return { ok: true, value: await handler.call(service, payload ?? {}) }
        } catch (error) {
          return { ok: false, error: { code: 'handler-failed', message: String((error && error.message) || error) } }
        }
      }
      const dispose = connection.rpc.handle(RPC_CHANNEL, (endpoint, payload) => dispatch(endpoint, payload), { authority: 'loopback' })
      return () => { void dispose() }
    }, 'agent-persona: rpc channel')
  } catch (error) {
