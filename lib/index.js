 ...
 *
 * Storage: `$DSH_HOME/workspace-personas.json`, re-read on every assembly when
 * its `mtimeNs:size` stamp changes, so edits apply to the next request.
 *
...
const STORE_PATH = join(DSH_HOME, 'workspace-personas.json')
...
const loadStore = (warn) => {
  let parsed
  try {
    parsed = JSON.parse(readFileSync(STORE_PATH, 'utf8'))
  } catch (error) {
    if (error?.code !== 'ENOENT') warn(`could not read ${STORE_PATH}; starting from an empty store`, error)
    return emptyStore()
  }
...
export function apply(ctx, config = {}) {
  ...
  let store = loadStore(warn)
  let storeStamp = stampOf()
  function stampOf() {
    try {
      const stat = statSync(STORE_PATH, { bigint: true })
  ...
  try {
    ...
    info(`section "${SECTION_NAME}" registered at order ${order} (store: ${STORE_PATH})`)
  } catch (error) {
  ...
  try {
    writeFileSync(join(DSH_HOME, 'workspace-persona.state.json'), `${JSON.stringify({
