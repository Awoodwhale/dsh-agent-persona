...
export const clipMarkdown = (text, maxChars) => {
  ...
  // A partial block makes the renderer look for fields the fragment does not have
  // ("Cannot read properties of undefined (reading 'code')" comes from a code block
  // whose content was cut away), so the cut is pulled back to a safe boundary:
  // the last line break, and never inside a fence or an inline code span.
  const lineBreak = cut.lastIndexOf('\n')
  if (lineBreak > maxChars * 0.6) cut = cut.slice(0, lineBreak)
