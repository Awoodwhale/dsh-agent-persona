...
export const resolvePersona = (personas, subject) => {
  // Most specific claim wins; the list order only breaks ties. A catch-all (no
  ...
  for (const persona of personas ?? []) {
    ...
    if (persona.targets.length === 0) {
      if (catchAll === undefined) catchAll = persona
      continue
    }
  ...
  if (catchAll !== undefined) return { persona: catchAll, reason: 'catch-all', matchedBy: [], specificity: 0 }
