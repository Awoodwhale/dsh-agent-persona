...
| 界面 | 位置 | 用途 |
|---|---|---|
| **设置页** | 设置 → Agent 人设 | 新建/编辑/删除/复制/排序人设；写正文、Markdown 预览、让 AI 改一版 |
...
三个偏好开关（`编辑后自动保存` / `在对话页显示「人设」标签` / `注册到 dsh-better-sidebar`）在
**管理页**与**设置 → 通用设置**里各有一份 —— 后一个位置不受这些开关影响，所以关掉显示位置后永远能从那里打开。
...
- **人设**：这条会话命中了哪条人设、为什么命中（位置命中还是默认人设）、正文（Markdown 渲染），
  以及一个就地编辑器（名称、默认人设开关、注入方式、正文）；改动后卡片头会出现「未保存 · 点击保存」。
...
- **测试的边界**：仓库里的测试是**单元测试 + 源码审计**（134 条宿主断言 + 15 条客户端断言 + 生成产物漂移校验），
