...
- 下拉框里的工作区来自 DSH 的工作区列表（带会话数），会话来自 DSH 的全部会话，按时间倒序，
  ...
  标题取自 DSH 自己的会话投影（和侧边栏显示的是同一个）；还没有标题的新会话，用它的**第一次输入**
  （再不行用**最近一次输入**）当标签 —— 这三个值来自**同一次**文件读取，不会去翻会话日志。
- 想用前缀、正则这类高级匹配，选 **自己输入…** 就会变成输入框。
- 折叠状态下，卡片**第二行**分开列出这条人设用在了哪些**工作区**、哪些**会话**（用工作区名与会话标题，
  鼠标悬停能看到原始的路径 / 规则）；一条位置都没加时写「默认：所有没被别的 Agent 人设占用的会话」。
- **越具体越优先**：会话 ID 的规则优先于工作区，精确优先于前缀/正则；只有当两条规则的"具体度"相同时，才按列表顺序取靠上的那条（顺序在卡片的 `⋯` 菜单里调）。
  例如：一条人设按目录 `/work/project` 命中、另一条按会话 ID 精确命中这条对话 —— 后者生效，哪怕它排在列表下面。
...
选中一条会话之后，旁边那个按钮可以**看这条会话聊了什么**：左右气泡式的对话（你的输入在右、Agent 的回复在左），
先读一段（大约几百条事件）显示出来，底部会写「已显示 N 条」；想看更早就点「继续加载」——不会一上来就把整个会话读完。

**只显示你真正敲进去的内容**：DSH 里那些"看起来像用户消息"的东西（工作区指令 `AGENTS.md`、运行时上下文快照、
skill 目录、goal 轮次说明）都是插件注入的，实测一条 `AGENTS.md` 注入就有 1.3 万字。这些一律不展示，
弹窗顶部只写「已隐藏 N 条插件注入内容」，不占位置、也不会让你困惑"这条我没说过"。

每条消息最长 4000 字（超了标「已截断」），下拉里每行右侧还会跟着**短 session id 和多久以前**，
标题一样的两条会话也能分得清。
